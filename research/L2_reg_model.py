...
for lammy in lammies:
    model = linear_model.Ridge(alpha=lammy)
    kf = model_selection.KFold(n_splits=5, shuffle=True, random_state=9842)
...
for lammy in lammies:
    model = linear_model.Ridge(alpha=lammy)
    kf = model_selection.KFold(n_splits=5, shuffle=True, random_state=93)
...
sk_preds.to_csv("predictions/preds_linreg_L2reg.csv", index=False)